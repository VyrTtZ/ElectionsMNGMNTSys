...
public class ElectionListPage {
            ...
            for (Election e : currentElectionList) {
                if (changeSortingType.isSelected())
                    if (e.getLocation().contains(searchBar.getText()))
                        newList.add(e);
                    else if (e.getName().contains(searchBar.getText()))
                        newList.add(e);
            }
            ...
            for (Politician p : currentPoliticianList) {
                if (changeSortingType.isSelected())
                    if (p.getHomeCounty().contains(searchBar.getText()))
                        newList.add(p);
                    else if (p.getName().contains(searchBar.getText()))
                        newList.add(p);
            }
        ...
        for (int i = 0; i < list.size() - 1; i++) {
            if (election)
                if (compareElections((Election) list.get(i), (Election) list.get(i + 1), criteria) < 0)
                    return false;
                else if (comparePoliticians((Politician) list.get(i), (Politician) list.get(i + 1), criteria) < 0)
                    return false;
        }
        return true;
    }
    ...
    public void save() throws Exception {
        ...
        ObjectOutputStream out=xstream.createObjectOutputStream(new FileWriter("electionListPage.fxml"));
        out.writeObject(mainElectionList);
        out.close();
    }

    public void load() throws Exception {
        Class<?>[] classes = new Class[] { mNodeL.class, mLinkedList.class, Candidate.class, Election.class, Politician.class};
        ...
        ObjectInputStream in = xstream.createObjectInputStream(new FileReader("electionListPage.fxml"));
